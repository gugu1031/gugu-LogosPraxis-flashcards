...
备考过程中，我困惑于市面上玲琅满目的闪卡软件，想着干脆开发一款适合自己的就好了~由此，LogosPraxis——咕咕我由个人备考需求引出而开发的网页和APP诞生啦！它用于将间隔重复闪卡、本地 PDF 精读、全文检索、结构化知识库与学习统计整合在同一套离线数据模型中。应用不要求注册账号，用户导入的文献、批注、卡片和复习记录默认只保存在当前浏览器或应用沙盒。
...
目前这个项目还在完善中，仅供个人使用，私自模仿或借鉴都是不允许的！但是可以事先问问我~嘻嘻QQ1987278499
